...
public class AllocationDAO extends BaseDAO {
    ...
    public List<Allocation> getAllProject() throws SQLException {
        ...
        try (PreparedStatement pre = getConnection().prepareStatement(str); ResultSet rs = pre.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                temp.setProject(pdao.setProjectInfor(rs)); // Assuming pdao is properly instantiated
    ...
    private Allocation setAllocationInfor(ResultSet rs) throws SQLException {
        ...
        temp.setUser(udao.getActiveUserByIdNull(rs.getInt(1))); // Assuming udao is properly instantiated
        temp.setProject(pdao.getById(rs.getInt(2))); // Assuming pdao is properly instantiated
    ...
    private Allocation setAllocationInforWithSt(ResultSet rs) throws SQLException {
        ...
        temp.setEffortRate(rs.getInt(5));
        temp.setStatus(rs.getBoolean(6));
        temp.setRole(this.getRoleNameOfAllocation(rs.getInt(7))); // Assuming getRoleNameOfAllocation() is defined
        return temp;
    }
    ...
    public void add(Allocation allocation, int[] memberIds) throws SQLException {
        String str = """
                     ...
                     (`userId`, `projectId`, `startDate`, `endDate`, `effortRate`, `role`, `status`)
                     VALUES (?, ?, ?, ?, ?, ?, ?);""";
        try (PreparedStatement pre = getConnection().prepareStatement(str)) {
            for (int i : memberIds) {
                pre.setInt(1, i);
